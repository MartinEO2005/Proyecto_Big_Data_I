...
def run_all(num_images=None):
    ...
    print("  ✅ Bloque económico completado.")
    ...
    for i in range(REINTENTOS):
        try:
            ...
            if os.path.exists(munis_path):
                ...
                conectividad_downloader.fetch_conectividad_historica_and_save(
                    munis_path,
                    base_outdir=BASE_DIR
                )
            break
    ...
    for i in range(REINTENTOS):
        try:
            ...
            df_st = osm_metrics.fetch_all_stations(
                gdf_munis,
                max_tile_area_deg2=getattr(osm_metrics, "MAX_TILE_AREA_DEG2", 1.0)
            )
